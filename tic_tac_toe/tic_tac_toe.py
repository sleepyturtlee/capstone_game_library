...
#NOTE: K added. window offset in the x direction
w_offset = 350
...
def user_click():
    ...
    # the -3 at the end is the offset. we moved it from the left --> mid. 
    # col = int(x // (width / 3)) + 1
    col = 0
    if x > 350 and x < 485:
        col=1
...
def handle_game_over_click():
    ...
    if 60+w_offset <= x <= 180+w_offset and height + 50 <= y <= height + 90:
        reset_game()
    elif 220+w_offset <= x <= 340+w_offset and height + 50 <= y <= height + 90:
        pygame.quit()
        sys.exit()
...
def run():
    ...
    while running:
        ...
        # copy everything from the screen (above) onto a temporary surface
